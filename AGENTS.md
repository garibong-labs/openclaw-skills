...
- `acp-host-transport.mjs`, `acp-host-transport-cli.mjs`, and `acp-host-transport-runner.mjs` — the POSIX/tmux production host transport. A private `prepare` call creates one detached-but-owned tmux session and returns its exact handle before activation; a separate `activate` call writes the activation record into that exact PTY. `status`, `ack-report`, `cancel`, and `reconcile` use only the retained transport record and handle. Record mutations use private sibling-temp plus atomic rename; status cursors have a bounded exact reissue path; acknowledgements re-derive canonical report content from stored reporting identity and verify its SHA-256 attestation before publication can close.
