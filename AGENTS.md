...
- `acp-report-pump.mjs` — the content-attested in-process pump imported by the installed controller plugin. Each invocation binds the exact transport/handle/destination/job identity, performs `claim-report` → fresh canonical derivation → `begin-delivery`, and returns the fenced delivery result. `acp-report-controller-preparation.mjs` owns private token generation, exact job construction, structural attestation, registration shape, ordered prepare/register/activate/commit flow, preactivation abort rollback, and commit-only recovery; its public automation template must stay byte-for-byte aligned with plugin commit `7cdaf6463aac5c34868162f89476295a2a2f90ca`.
