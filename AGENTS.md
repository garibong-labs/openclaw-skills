...
- `acp-reporting-contract.mjs` — the pure reporting-contract validator. Current schema `acp-reporting-v2`; the legacy `acp-reporting-v1` shape is accepted only for canonical agent `claude` during a bounded migration and is rejected for every other agent. It owns the closed, fail-closed agent presentation mapping (`claude` → `Claude Code`, `codex` → `Codex`): the public harness label on reporting templates is derived from the canonical config `agent`, never from caller data, and `invalid_reporting_agent` remains its own boundary code when the validator is invoked directly or a v2 bundle's `agent` attestation mismatches. It also owns `buildAcpStartMessage`, the one production preparation path for the 13-line round-start message: the round title (round 1 vs 수정 라운드 N) and the harness label are derived from structured inputs and self-checked through the same validator, never caller-supplied.
- `acpx-runtime-preflight.mjs` and `acpx-runtime-preflight-cli.mjs` — the canonical machine-enforced runtime-module preflight. The `attest` action consumes the owner-private structured output of `openclaw plugins info acpx --json`, uniquely selects the dependency named exactly `acpx` (rejecting missing/duplicate exact matches, relative or malformed paths, and the active plugin package root — the manifest-name gate rejects `@openclaw/acpx` even when no root field is derivable), validates the package (name, ACPX ≥ 0.11.2 release version, real `dist/runtime.js`, and the authoritative `createAcpRuntime`/`createRuntimeStore`/`createAgentRegistry` capability exports), and writes an owner-private digest-bound attestation (`acpx-runtime-attestation.v1`). The `assemble` action re-verifies that attestation fail-closed (absent/stale/mismatched/invalid) and writes the final supervisor config with the attested `runtimeModule`, replacing the template sentinel `RUNTIME_MODULE_FROM_PREFLIGHT` — callers never hand-copy or choose `runtimeModule`; already-prepared configs with a validated absolute path stay valid. Bounded stable codes only; no path or plugin-info payload is ever echoed.
- `acp-start-message-cli.mjs` — the operator CLI for `buildAcpStartMessage`: reads one absolute owner-private JSON input (`--input`), writes only the rendered start message to stdout, and fails closed with one `start_message_builder_error` event (bounded stable code, no path or free-text echo) on stderr and exit 64.
- `acp-host-transport.mjs`, `acp-host-transport-cli.mjs`, and `acp-host-transport-runner.mjs` — the POSIX/tmux production host transport. A private `prepare` call creates one detached-but-owned tmux session and returns its exact handle before activation; a separate `activate` call writes the activation record into that exact PTY. `status`, `cancel`, and `reconcile` use only the retained transport record and handle. This route exists for host surfaces that expose only bounded one-shot shell calls and no writable unified-exec session.
