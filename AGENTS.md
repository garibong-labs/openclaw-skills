...
node --test acp-discord-orchestrator/scripts/test-acp-report-controller-preparation.mjs
...
# acp-discord-orchestrator — installed-boundary evidence probe (not a node:test file)
node acp-discord-orchestrator/scripts/check-openclaw-automation-boundary.mjs
```
...
`check-openclaw-automation-boundary.mjs` is deliberately outside the node:test suites: it executes the *installed* OpenClaw 2026.8.1 bundles to pin the three model-callable behaviours the portable mocks cannot see — that `capCronJobToolsAllow` re-emits a finite `toolsAllow` in creator tool-surface order rather than the requested order, that headless scripts run under `agent:main:cron:<jobId>:trigger`, and that a successful `automations(action:"remove")` reaches the model as `{ content, details: { removed: true } }` with no top-level `removed`. It locates bundles by content signature (never by hashed filename or whole-bundle hash) and reports SKIP with exit 0 when no matching install is present, so it never fails a machine that has nothing to probe. Report its check count separately from the node:test totals.
...
- `acp-reporting-contract.mjs` — the pure reporting-contract validator and canonical start/intermediate/terminal message builder. Current schema `acp-reporting-v3`, whose enabled `reportPump` binds the exact job id plus the non-secret deterministic controller-script structure (version/digest, 60-second timeout, five-call allowlist, delivery none) without carrying the substituted script, lease token, or a static report. `acp-reporting-v2` (watchdog shape) stays accepted for already-prepared configs, and legacy v1 is Claude-only. It owns the closed agent presentation mapping and model/report identity rules.
...
- `acp-report-pump.mjs` — the content-attested in-process pump imported by the installed controller plugin. Each invocation binds the exact transport/handle/destination/job identity, performs `claim-report` → fresh canonical derivation → `begin-delivery`, and returns the fenced delivery result plus the exact structured report input used for rendering, so acknowledgement never re-derives minute-sensitive fields. `acp-report-controller-preparation.mjs` owns private token/declaration-key generation, the two-stage model-callable job construction (a disabled inert `automations` **add** with no reserved `id` — the public `cron.add` schema has none — then one **update** on the scheduler-returned exact id that substitutes the token plus that id and enables the job atomically, with a bounded declaration-key replay for a lost add response), structural attestation, registration shape, ordered prepare/register/activate/commit flow, bounded exact registration replay/recovery, preactivation abort rollback, and commit-only recovery. It attests the persisted job's `toolsAllow` as an exact duplicate-free set rather than an ordered list, because the model-callable boundary normalizes the order away, and it reads removal success through one strict bounded parser covering the wrapped `details.removed` envelope that boundary actually returns. Its public automation template must stay byte-for-byte aligned with the controller plugin template. The production loader enforces the template and placeholder-script SHA-256 constants exported from `acp-reporting-contract.mjs`; the normative runtime contract carries the exact plugin implementation pin, which must be replaced with the companion plugin's merge commit before a compatibility branch merges. Prepared controller ticks are inert so registration recovery remains owner-driven; every non-throwing controller-script path returns the bounded plain object `{}` required by OpenClaw 2026.8.1 scheduler result parsing.
