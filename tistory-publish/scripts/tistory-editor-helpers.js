/**
 * tistory-editor-helpers.js — 티스토리 에디터 조작 헬퍼
 *
 ...
 *
 * 순서:
 *   1. setCategory(categoryName) — 카테고리 선택
 *   2. setTitle(title)           — 제목 입력
 *   3. insertContent(html)       — 본문 HTML 삽입 (OG placeholder 포함)
 *   4. getOGPlaceholders()       — OG URL 목록 추출
 *   5. prepareOGPlaceholder(url) + Playwright Enter — OG 카드 생성
 *   6. setRepresentImageFromEditor() — 대표이미지 설정
 *   7. setTags(tags)             — 태그 입력
 *   8. clickComplete() → clickPublish() — 완료 → 발행
 */
...
function buildBlogHTML({ intro, articles }) {
  ...
  articles.forEach((article, i) => {
    ...
    html += article.body + '\n';
    // 코멘트: 각 기사 안에 삽입
    html += `<p data-ke-size="size16"><b>${article.commentLabel || '코멘트'}</b> - ${article.comment}`;
    // 마지막 기사면 "끝." 붙이기
 ...
 */
function getSafeTinyEditor() {
  if (typeof tinymce === 'undefined' || !tinymce || !tinymce.activeEditor) return null;
...
function registerSchema() {
  ...
  if (!editor || !editor.schema) {
    return 'schema skipped: tinymce unavailable';
  }
...
function insertContent(html) {
  ...
  document.querySelectorAll('textarea').forEach(t => {
    if (t.value.length > 5000 || !t.value) t.value = content;
  });
...
function verifyOGCard(url) {
  ...
  if (!editor || !editor.getBody) {
    return { found: false, ogCardCount: 0, cleaned: false, note: 'tinymce unavailable' };
  }
...
function replaceOGPlaceholder(url) {
  ...
  const editor = getSafeTinyEditor();
  if (!editor) return 'tinymce unavailable';
...
function cleanupOGResiduals() {
  const editor = getSafeTinyEditor();
  if (!editor || !editor.getBody) {
    return { ogCards: 0, pendingRemoved: 0, rawUrlsRemoved: 0, blankParasRemoved: 0, note: 'tinymce unavailable' };
  }
  const body = editor.getBody();
  ...
  ogFigures.forEach(fig => {
    ...
    while (next && next.tagName === 'P') {
      const normalizedText = (next.textContent || '').replace(/[\u00a0\u200b\s]/g, '');
      const normalizedHtml = (next.innerHTML || '')
        .replace(/<br\s*\/?>/gi, '')
        .replace(/&nbsp;|&#160;|&#8203;/gi, '')
        .replace(/[\u00a0\u200b\s]/g, '')
        .trim();
  ...
  return { ogCards, pendingRemoved, rawUrlsRemoved, blankParasRemoved };
}

function insertOGCard(url) {
  const editor = getSafeTinyEditor();
  if (!editor || !editor.getBody) return 'tinymce unavailable';
 ...
 *
 * @param {string[]} tags - 태그 배열 (예: ["OpenClaw", "릴리즈노트"])
...
/**
 * 티스토리 에디터의 사진 업로드 input으로 로컬 이미지를 올리고,
 * 본문 마지막에 생성된 이미지 figure/img를 현재 커서 위치로 이동시킨다.
 *
 * @param {string[]} files - 로컬 파일 경로 배열
 * @returns {Promise<Object>} 업로드 결과
 */
async function uploadInlineImages(files) {
  const input = document.querySelector('#openFile');
  if (!input) return { success: false, error: 'file input not found' };
  if (!Array.isArray(files) || files.length === 0) return { success: false, error: 'no files' };

  const beforeFigures = document.querySelectorAll('figure, img').length;
  return {
    success: true,
    pending: true,
    filesCount: files.length,
    beforeFigures,
    uploadInputSelector: '#openFile',
    note: 'Use Playwright set_input_files on #openFile, then call moveLastImageAfterMarker(marker) for each uploaded image.'
  };
}

/**
 * 방금 업로드된 마지막 이미지 블록을 찾아 특정 placeholder 뒤로 이동.
 * placeholder는 <p data-image-marker="name"></p> 형태를 사용한다.
 *
 * @param {string} markerName
 * @returns {string}
 */
function normalizeImagePresentation(node, options = {}) {
  if (!node) return null;
  const figure = node.matches && node.matches('figure[data-ke-type="image"]')
    ? node
    : (node.closest && node.closest('figure[data-ke-type="image"]')) || node;
  const img = figure.querySelector ? figure.querySelector('img') : (figure.tagName === 'IMG' ? figure : null);
...
function moveImageByFilenameAfterMarker(filename, markerName, options = {}) {
  ...
  if (!target) {
    ...
    target = imgs.find(img => {
      const name = img.getAttribute('data-filename') || img.getAttribute('src') || '';
      return name.includes(filename);
    }) || null;
  }
  if (!target) return 'image block not found';

  const wrapper = document.createElement('div');
  wrapper.innerHTML = target.outerHTML || target.innerHTML || '';
  ...
  document.querySelectorAll('textarea').forEach(t => {
    if (t.value.length > 100) t.value = editor.getContent();
  });
  editor.setDirty(true);
  editor.fire('change');
  editor.save();

  return cloned.outerHTML || 'ok';
}

function buildImageMarker(markerName) {
  return `<p data-ke-size="size16" data-image-marker="${markerName}">&#8203;</p>`;
}


/**
 * 업로드된 이미지 블록들을 filename 기준으로 marker 뒤에 이동한다.
 * mapping 예: [{marker:"trend-image-1", filename:"foo.jpg"}]
 */
function moveImagesToMarkers(mapping) {
  const editor = getSafeTinyEditor();
  if (!editor || !editor.getBody) return [{ result: 'tinymce unavailable' }];
  const body = editor.getBody();
  const results = [];

  const refreshDetachedFigures = () => Array.from(body.querySelectorAll('figure[data-ke-type="image"]')).filter(fig => {
    return !fig.previousElementSibling || !fig.previousElementSibling.matches('[data-image-marker]');
  });

  for (let idx = 0; idx < mapping.length; idx++) {
    const item = mapping[idx];
    const options = imagePresentationOptions(item);
    let result = moveImageByFilenameAfterMarker(item.filename, item.marker, options);

    if (result === 'image block not found' || result === 'clone failed') {
      const marker = body.querySelector(`[data-image-marker="${item.marker}"]`);
      const detachedFigures = refreshDetachedFigures();
    ...
    results.push({ marker: item.marker, filename: item.filename, result });
  }

  const markers = mapping.map(item => item.marker);
  const postCheck = markers.map(marker => {
    const markerNode = body.querySelector(`[data-image-marker="${marker}"]`);
    const next = markerNode ? markerNode.nextElementSibling : null;
    return {
      marker,
      markerStillExists: !!markerNode,
      nextTag: next ? next.tagName : null,
      nextIsImageFigure: !!(next && next.matches && next.matches('figure[data-ke-type="image"]'))
    };
  });
  ...
  return { results, postCheck, figures: body.querySelectorAll('figure[data-ke-type="image"]').length };
}

function debugImageBlocks() {
  const editor = getSafeTinyEditor();
  if (!editor || !editor.getBody) {
    return [];
  }
  const body = editor.getBody();
  return Array.from(body.querySelectorAll('figure, img')).map((el, i) => ({
    idx: i,
    tag: el.tagName,
...
function collectUploadedImageFiguresByFilename(filenames) {
  const editor = getSafeTinyEditor();
  if (!editor || !editor.getBody) {
    return filenames.map((filename) => ({ filename, outerHTML: null, found: false }));
  }
  const body = editor.getBody();
  const figures = Array.from(body.querySelectorAll('figure[data-ke-type="image"]'));
  return filenames.map((filename) => {
    ...
    return {
      filename,
      outerHTML: fig ? (fig.outerHTML || '') : null,
      found: !!fig,
    };
  });
}

function rebuildContentWithImageFigures(sectionMarkers) {
  const editor = getSafeTinyEditor();
  ...
  return { success: true, contentLength: editor.getContent().length };
}
