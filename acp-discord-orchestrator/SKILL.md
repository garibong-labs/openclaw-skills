...
Conversation blocking belongs to the caller. Use separate bounded `status` calls with the same `transportFile` and `processHandle`, carrying the previous `lastSequence` as `afterSequence`. Every successful status returns a private `serviceCursor`; before the next status, service newly steered input in the control conversation and include `serviceCursorAck` with that exact cursor, the exact control conversation ID, and the fresh service instant. This is caller attestation, not a Discord network proof, because the transport has no chat credentials. A missing, wrong-destination, wrong-cursor, future, or stale attestation fails closed and prevents a tight polling loop from starving new user input. Poll at 1, 2, 4, and then 5 seconds, capped at five seconds. Do not read the event, stderr, or exit files directly; the transport validates and bounds them.
...
At exactly 600 seconds of ACP elapsed time, `status` returns a private `reportPublication` boundary for cadence 1. The first boundary is `report_required`; the stored state then becomes `publication_pending`, and later active polls cannot advance beyond that evidence sequence. Build the public message from structured fields with `scripts/acp-report-message-cli.mjs` and [templates/intermediate-report-input.json](templates/intermediate-report-input.json), deliver it to the exact control conversation, then call the closed `ack-report` action with the exact report ID/kind/cadence and a fresh caller-attested Discord receipt. Direct source delivery attests `deliveryStatus: "sent"`; isolated announce delivery attests `"delivered"`; no other value succeeds. Each next cadence advances exactly ten minutes from the prior due anchor, so a late publication never resets ACP elapsed cadence.

The intermediate time line is structured, exactly `⏱️ **ACP 시간**: 전체 <N분> · 현재 단계 <N분> · 마지막 ACP 활동 <N분 전>`. `마지막 ACP 활동` is the age of the intermediate boundary's `lastAcpActivityAt` — the latest normalized ACP model/tool/status/activity event of the current exact execution handle. It is never reset by a transport poll boundary, report rendering or delivery receipt, phase bookkeeping, owner-side work, or a delta/result-cursor update. `Δ` in the 새 결과 bullet is independent of that age: the boundary's `newResultDelta` counts only ACP results completed after the last successfully delivered intermediate report. Ongoing ACP activity without a newly completed result keeps `Δ0` and renders `Δ0 · 새로 확인된 ACP 결과 없음`, so `마지막 ACP 활동 0분 전` with `Δ0` is a valid combination; a newly completed result increments `Δ` regardless of the activity age. The builder rejects the legacy free-text intermediate `elapsed` input.

A terminal event before a cadence requires only a terminal report. A terminal event racing an overdue intermediate supersedes it and changes the required identity to terminal cadence 0. Build that message with [templates/terminal-report-input.json](templates/terminal-report-input.json). Exit reconciliation may complete privately, but transport status remains `terminal_publication_pending` and owner verification must not begin until the exact terminal receipt is acknowledged. Receipt conversation/message IDs and delivery instants stay in the private record and are never returned by the transport or inserted into normalized events.
...
After the mapped exit exists, call the host transport CLI with `action: "reconcile"`, the same private transport file, and the same handle. It derives the exact lifecycle ledger and confirms terminal intent plus mapped exit, but returns `terminal_publication_pending` until terminal `ack-report` succeeds; a later reconcile then returns the reconciled state. A supervisor error before activation is reconciled with the ledger's explicit null handle; an invented handle is never bound to that run. If an activated transport disappears without terminal intent, use `scripts/acp-lifecycle-reconcile-cli.mjs` with the `tracking_lost` outcome. Handle mismatch, missing terminal evidence, and exit mismatch fail closed.
