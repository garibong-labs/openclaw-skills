...
The transport record also owns the fail-closed publication state machine `report_required -> publication_pending -> receipt_acked`. Every mutation after record creation writes a mode-0600 private sibling temp file and atomically renames it over the authoritative record; a failed write or rename leaves the old record intact rather than truncate-corrupted. The activation-confirmed event binds the supervisor's original elapsed-time origin, and the first intermediate becomes due at exactly ACP elapsed 600,000 ms rather than 600,000 ms after the activation call. A fresh exact-handle `status` at that instant exposes one private report identity and the evidence boundary, then persists `publication_pending`; later polls cannot return a newer sequence until `ack-report` accepts the matching identity, kind `intermediate`, cadence, exact control-conversation destination, 1–32 digit decimal message ID, and a delivery instant at or after the requirement with one of the closed successful statuses: `"sent"` for a direct source send or `"delivered"` for isolated announce delivery. The caller-attested delivery instant preserves the documented bounded ISO-8601 wire spelling: an explicit `Z` or numeric offset and at most six fractional-second digits, including Discord forms such as `2026-08-22T07:47:48.530000+00:00`. Because it comes from a remote chat-provider clock, receipt freshness accepts only a symmetric one-second skew around the host boundaries: no earlier than one second before the required instant and no later than one second after the current host instant; the five-minute receipt-age limit remains fail-closed. Private transport-generated timestamps (`createdAt`, cadence anchors, publication requirements, and `controlCursorIssuedAt`) remain canonical millisecond UTC strings generated by `Date#toISOString`. `ack-report` also carries the exact structured report and a `messageDigest`: the transport checks its stored agent/model/round/repository/branch identity — for Codex, the same deterministic `model[reasoningEffort]` public identity used by start/report builders and the normalized `started` event — re-runs the matching canonical builder, binds terminal status when applicable, and requires SHA-256 of that exact canonical string (excluding the report CLI's final stdout newline). Canonical construction is therefore machine-enforced. No current obligation fails `host_transport_report_not_required`; retrying an already acknowledged identity fails the distinct `host_transport_report_ack_duplicate`. Acknowledged message IDs remain private, unique, and bounded to the most recent 64. A valid late intermediate acknowledgment advances from the prior due anchor in one transition past all already-missed anchors, returns the bounded `skippedCadences` count, and schedules the first future cadence while ACP `elapsedMs` continues from the original supervisor start.
...
```json
{"schemaVersion":"acp-host-transport.v1","action":"status","transportFile":"/absolute/private/transport.json","processHandle":"<exact-host-handle>","afterSequence":12,"serviceCursorAck":{"cursor":"<previous-service-cursor>","conversationId":"<exact-control-conversation>","servicedAt":"2026-08-31T10:00:00.000Z"}}
```

```json
{"schemaVersion":"acp-host-transport.v1","action":"status","transportFile":"/absolute/private/transport.json","processHandle":"<exact-host-handle>","afterSequence":12,"reissueServiceCursor":true}
```

The complete exact-key `ack-report` example is [templates/host-transport-ack-report.json](../templates/host-transport-ack-report.json); its `reportKind` selects the exact intermediate or terminal builder input shape.
...
- poll only the transport `status` action with those same values, waiting 1, 2, 4, and then 5 seconds, capped at five seconds, and acknowledge each returned service cursor before the next poll;
- service steered control-surface input at each returned poll boundary and continue the same turn unless the message explicitly cancels or replaces it; the cursor is caller attestation rather than network proof;
- publish and acknowledge every required intermediate report before advancing past its evidence boundary; let terminal supersede an overdue intermediate;
