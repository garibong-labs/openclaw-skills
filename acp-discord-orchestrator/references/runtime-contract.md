...
- `model`: explicit ACP model ID of at most 256 characters, passed to the runtime's `sessionOptions.model` verbatim. The grammar is the generic identifier grammar (alphanumeric start, then alphanumerics and `_ . : / @ + -`), optionally followed by exactly one bracket suffix at the very end: one non-empty alphanumeric run in matched square brackets. The bracketed form is valid on purpose — ACPX 0.11.2+ adapters advertise Codex reasoning selection inside the model ID itself, so adapter-advertised IDs such as `gpt-5.2[high]`, `gpt-5.6-sol[low]`, or `claude-fable-5[1m]` are complete model IDs, not a model plus a separate thinking option, and the supervisor has no separate thinking field. The complete accepted string appears unchanged in `sessionOptions.model`, the normalized `started` event, and the reporting metadata identity lines. Malformed forms fail closed as `invalid_model`: an empty suffix, unmatched or nested brackets, more than one suffix, characters after the closing bracket, whitespace, control characters, backticks, or any punctuation outside the grammar. Agent names and every other identifier keep the strict bracket-free grammar. The value lives in the ACP adapter's model namespace, not in OpenClaw's provider/catalog namespace: for `agent: "claude"` the correct value is the ID the Claude ACP adapter advertises — `claude-fable-5`, `claude-opus-5`, or a bracketed form like `claude-fable-5[1m]` — while the OpenClaw provider key `anthropic/claude-fable-5` names an OpenClaw catalog route the adapter does not accept. A Claude `model` beginning with the known OpenClaw provider prefix `anthropic/` (any letter case) fails closed with the stable code `invalid_model_openclaw_provider_key` at the shared model-resolution boundary — before dynamic runtime import, probing, or adapter startup — on both paths, keeping each path's exit mapping: the config loader rejects it as invalid config (exit 64), and a config assembled in memory fails `runSupervisor` preflight with a `supervisor_error` event (exit 22). The guard is scoped to canonical `claude` and this one prefix; slash-containing model IDs for other agents or providers are not affected. Omission is agent-specific. For `agent: "codex"`, an omitted `model` is normalized by the supervisor to the single authoritative explicit Codex default `gpt-5.6-sol[medium]` (`CODEX_DEFAULT_MODEL`) — never inherited from the adapter/backend preset — before the reporting bundle is validated and before the runtime receives `sessionOptions.model`, so the loaded config, the reporting identity lines, the normalized `started` event, and the ACPX session all name `gpt-5.6-sol[medium]`, and a Codex reporting bundle claiming `runtime-default` fails closed as `invalid_reporting_start_message`. For `agent: "claude"`, omission stays genuine: the run identifies itself with the literal `runtime-default` label — the `started` event emits it and the reporting templates must embed it on their ACP identity lines
