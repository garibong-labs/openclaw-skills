...
An intermediate `reportPublication` boundary additionally carries two independent derived values. `lastAcpActivityAt` is the timestamp of the latest normalized ACP model/tool/status/activity event of the exact execution handle (envelope type `activity`, with the handle's own `activation_confirmed`/`started` lifecycle marks as the pre-runtime floor); it is recomputed from the append-only event log on every status, so it only moves forward with new ACP events and is never reset by a transport poll boundary, report rendering or delivery receipt, phase bookkeeping, owner-side work, or a result-cursor update — timer-driven `progress` bookkeeping does not move it either. `newResultDelta` counts completed ACP results (normalized tool events with a completed status) strictly after the private `resultBaselineSequence` result cursor and within the current report identity's evidence boundary. The cursor advances exactly once per successfully delivered intermediate report — to that report's own evidence boundary, during `ack-report` — and never touches the activity instant, so ACP activity without a newly completed result keeps `Δ0` while `마지막 ACP 활동` stays fresh, and a newly completed result increments the delta independently of the activity age.
