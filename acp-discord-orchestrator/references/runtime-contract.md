...
A matching normalized terminal event supersedes an overdue intermediate atomically and creates terminal cadence 0. An intermediate identity can no longer acknowledge it. Terminal before ten minutes creates no intermediate obligation. Once mapped exit evidence exists, reconciliation still records the lifecycle exit, but the transport returns `terminal_publication_pending` until the canonical terminal receipt is acknowledged; only then can a later reconciliation surface the ledger's closed state. Receipt conversation IDs, message IDs, delivery instants, report IDs, service cursors, and process handles never enter normalized events, and receipt identifiers are not echoed by `ack-report`.

Every successful `status` also issues one private control-service cursor. The next `status` must carry exact-key `serviceCursorAck` with that cursor, the exact control conversation, and a fresh ISO `servicedAt` not before cursor issuance. This is an enforceable caller-attestation gate against a tight polling loop starving newly steered user input; it is not a Discord network proof. The transport has no chat credentials and cannot verify that the caller actually fetched or answered a message.
...
```json
{"schemaVersion":"acp-host-transport.v1","action":"ack-report","transportFile":"/absolute/private/transport.json","processHandle":"<exact-host-handle>","reportId":"<exact-report-id>","reportKind":"intermediate","cadence":1,"receipt":{"conversationId":"<exact-control-conversation>","messageId":"<decimal-message-id>","deliveredAt":"2026-08-31T10:00:05.000Z","deliveryStatus":"delivered"}}
```
