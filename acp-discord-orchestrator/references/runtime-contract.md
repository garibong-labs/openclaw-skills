...
- `reasoningEffort`: Codex-only reasoning selection of at most 64 identifier characters. Omission on Codex resolves to `medium` (`CODEX_DEFAULT_REASONING_EFFORT`); presence on any other agent fails as `reasoning_effort_unsupported_agent`. After the base model acknowledgement, the supervisor applies this value through runtime config option `thought_level`; failure maps to `codex_reasoning_config_apply_failed`. The base model write maps failures to `codex_model_config_apply_failed`. Both writes must resolve before `startTurn`, so no ACP prompt can run with an unacknowledged selection. Public reporting derives the deterministic display identity `model[reasoningEffort]` — default `gpt-5.6-sol[medium]` — from the two validated fields; that composite value appears in reporting and the normalized `started` event but is never sent back as a runtime model. A Codex reporting bundle claiming `runtime-default` fails closed as `invalid_reporting_start_message`.
