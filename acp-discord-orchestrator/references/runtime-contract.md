...
- `agent`: canonical ACP agent name from the closed supported set: `claude` or `codex`. The public presentation mapping is closed and fail-closed — `claude` is presented as `Claude Code` and `codex` as `Codex` on every reporting template, and a caller can never choose or introduce a label through config data. ACPX normalizes agent names with trim/lowercase, so a spelling such as `Claude` or `Codex` would still resolve to the same adapter while bypassing every exact-match gate; any spelling that normalizes to a supported agent other than the canonical lowercase value is rejected as `invalid_agent_not_canonical`. An agent outside the closed set fails the reporting contract with `invalid_reporting_agent` during config loading — before dynamic runtime import, probing, or adapter startup — and the in-memory `runReportingPreflight` backstop enforces the same rejection for configs assembled without the loader
